...
def build_dashboard_for_user(user: User) -> dict:
    """Shared dashboard logic used by both API + UI."""
    today = date.today()
    ws = week_start(today)
    we = ws + timedelta(days=7)

    runs_all = Run.query.filter(Run.user_id == user.id).order_by(Run.run_date.asc()).all()
    run_dates = {r.run_date for r in runs_all}
...
def create_app():
    ...
    def ensure_own_page(username: str):
        """Block viewing/logging for other users."""
        me = current_user()
        if not me:
            return redirect(url_for("login_page"))
    ...
    def login_post():
        ...
        # New user → create + force password setup
        if not user:
    ...
    def user_page(username):
        guard = ensure_own_page(username)
        if guard:
            return guard
        ...
        return render_template(
            ...
            weekly_done=dash["this_week"]["runs"],
            weekly_goal=dash["this_week"]["goal"],
            weekly_km=dash["this_week"]["distance_km"],
            motivation=dash["motivation"],
            next_run=dash["next_run"],
        )
    ...
    def ui_log_run(username):
        ...
        r = Run(user_id=user.id, run_date=d, distance_km=distance_km, duration_min=duration_min, mood=mood)
        db.session.add(r)
        db.session.commit()
